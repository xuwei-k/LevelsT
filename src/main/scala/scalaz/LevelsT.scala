...
object LevelsT extends LevelsTInstances {
  ...
  object T {
    implicit def equal[F[_]: Functor: Foldable, A: Order](implicit
        F: => Eq1[F],
        A: Equal[A]
  ...
  private case class Wrap[F[_], A](maybe: Maybe[T[F, A]])
  private object Wrap {
    implicit def equal[F[_]: Functor: Foldable: Eq1, A: Order]
        : Equal[Wrap[F, A]] =
      (a1: Wrap[F, A], a2: Wrap[F, A]) => {
        (a1.maybe, a2.maybe) match {
          case (Maybe.Just(x), Maybe.Just(y)) =>
            Equal[T[F, A]].equal(x, y)
          case (Maybe.Just(x), Maybe.Empty()) =>
            Foldable[T[F, *]].empty(x)
          case (Maybe.Empty(), Maybe.Just(y)) =>
            Foldable[T[F, *]].empty(y)
  ...
  implicit def equal[F[_]: Foldable: Functor, A](implicit
      A: Order[A],
      F: => Eq1[F]
  ): Equal[LevelsT[F, A]] = { (a1: LevelsT[F, A], a2: LevelsT[F, A]) =>
    {
      ...
      val f = Functor[F]
      f0.eq1[Wrap[F, A]]
        .equal(f.map(a1.run)(Wrap.apply), f.map(a2.run)(Wrap.apply))
    }
  }
